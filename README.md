...
* To install, just download the `local-grid-hub` JAR and run it:
  * `java -jar local-grid-hub-&lt;version&gt;.jar`
...
The easiest way to run the unit tests is from the command line. From the root folder of each `local-grid-parent` node module (e.g. - `local-firefox-node`):

```bash
$ mvn test -DskipTests=false # run Firefox node unit tests with default browser
$ mvn -Dselenium.browser.name=firefox.headless test -DskipTests=false # run unit tests with "headless" Firefox
```
...
* `grid-relay*.log` for relay server output
